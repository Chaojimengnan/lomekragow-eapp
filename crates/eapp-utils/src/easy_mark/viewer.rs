...
pub fn easy_mark(ui: &mut Ui, easy_mark: &str) {
    let items = easy_mark::Parser::new(easy_mark).collect::<Vec<_>>();
    easy_mark_it(ui, items.iter());
}

pub fn easy_mark_it<'em>(ui: &mut Ui, items: impl Iterator<Item = &'em easy_mark::Item>) {
...
pub fn item_ui(ui: &mut Ui, item: &easy_mark::Item) {
    let row_height = ui.text_style_height(&TextStyle::Body);
